...
void ListarPorGenero(WINDOW *EntradaInfo) {  //PRECISA SER COMPLETADO
    ...
    char *StringAux = (char*) malloc(sizeof(char) * 41);
    ...
    move(yterminal / 2 + 1, (xterminal - (xborda - 30)) / 2 + 2);
    refresh();
    getnstr(StringAux, 41);
    setbuf(stdin, NULL);
    char stringCopia [41];
    

    for(int a = 0; a < QuantidadeSeries; a++) {
         strcpy(stringCopia,serie[a].Genero);
        char *string2 = strtok(stringCopia, "/");

        if(strcasecmp(StringAux, string2) == 0) {
    ...
    while (1) {
        ...
        for (int a = OpcoesMin; a < OpcoesMax; a++) {
            ...
            int x = (41 - strlen(serie[IntAux].Nome)) / 2;
...
void PesquisarSerie(WINDOW *EntradaInfo) {
    ...
    char *StringAux1 = (char*) malloc(sizeof(char) * 101);
